...
collateral_evaluation(FirstClass,_,_,excellent) :-
        FirstClass >= 100.
collateral_evaluation(FirstClass,SecondClass,_,excellent) :-
...
select_value(collateral,profile(C,_,_),C).
select_value(finances,profile(_,F,_),F).
select_value(yield,profile(_,_,Y),Y).
